...
(() => {
  ...
  themeButton.addEventListener('click', () => {
    ...
    try {
      localStorage.setItem('igarassu-theme', theme);
    } catch {
      /* O modo funciona mesmo sem armazenamento. */
    }
    syncThemeButton();
  });
  ...
  nav.addEventListener('focusout', (event) => {
    if (!event.relatedTarget?.closest('.dropdown')) closeDropdown();
  });
  document.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'Tab' && menuOpen) {
      const focusable = [
        menuButton,
        ...$$('a[href],button', nav).filter((el) => el.getClientRects().length),
      ];
  ...
  $$('[data-config-link]').forEach((el) => {
    const key = el.dataset.configLink,
      url = safeUrl(config[key]);
    if (!url) return;
    if (key === 'whatsapp' && !['wa.me', 'api.whatsapp.com'].includes(url.hostname)) return;
    if (key === 'instagram' && !['instagram.com', 'www.instagram.com'].includes(url.hostname))
      return;
    el.href = url.href;
    el.target = '_blank';
  ...
  if (config.encarregadoDados && $('[data-dpo]'))
    $('[data-dpo]').textContent = config.encarregadoDados;
  ...
  if (institutional) {
    const fallback = () => {
      institutional.hidden = true;
      $('.institutional-fallback').hidden = false;
    };
    const showImage = () => {
      institutional.hidden = false;
      $('.institutional-fallback').hidden = true;
    };
  ...
  function validateField(el) {
    ...
    if (el.type === 'checkbox' ? !el.checked : !el.value.trim())
      error =
        el.type === 'checkbox'
          ? 'Confirme a ciência da Política de Privacidade.'
          : 'Preencha este campo.';
    else if (el.type === 'email' && el.validity.typeMismatch) error = 'Informe um e-mail válido.';
    else if (el.type === 'tel' && !/^\+?[\d\s().-]+$/.test(el.value))
      error = 'Informe um telefone válido.';
    else if (
      el.type === 'tel' &&
      (el.value.replace(/\D/g, '').length < 10 || el.value.replace(/\D/g, '').length > 15)
    )
  ...
  function submitContactForm(event) {
    ...
    gmail.search = new URLSearchParams({
      view: 'cm',
      fs: '1',
      to: 'serventiaregistraldeigarassu@gmail.com',
      su: subject,
      body,
    }).toString();
    $('#form-status').textContent =
      'Mensagem preparada. Conclua o envio no Gmail. Nenhuma mensagem foi enviada automaticamente. Se a nova aba não abrir, utilize o link abaixo.';
    let continueLink = $('#gmail-continue');
    if (!continueLink) {
      continueLink = document.createElement('a');
      continueLink.id = 'gmail-continue';
      continueLink.className = 'text-link';
